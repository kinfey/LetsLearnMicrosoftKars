...
第一次仓库提示注入测试六周后，Contoso Engineering 准备进行有限生产发布。团队
必须证明的不只是演示效果。另一名运维人员必须能够部署 Forge、解释每项权限、观察
每条拒绝路径并回滚系统。
...
1. Builder 接收 Issue 与固定 Source Revision。
2. 它只调用批准的仓库、Patch 与测试工具。
3. 工具流量只到达批准的研发 MCP 主机。
4. 模型请求在 Token 预算内经过路由器。
5. Builder 生成最小 Diff 与测试证据。
6. Reviewer 收到 Diff，但不会获得 Builder 写入权限。
7. Reviewer 批准、拒绝或要求新的 Patch。
...
最初的原型因为“能够修改代码”而令人印象深刻。生产设计之所以可信，是因为团队能够
