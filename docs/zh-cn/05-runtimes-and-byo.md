# 5. 把 Atlas 迁入受治理的 Runtime

## 一个令人不安的发现

策略原型使用 OpenClaw，但 Maya 最初的 Atlas 是带自定义报告解析的 Python
应用。完全重写会延误项目；原样运行旧容器又会保留直接凭据和网络假设。
...
```text
AZURE_OPENAI_API_KEY -> 由 Atlas 读取
AZURE_OPENAI_ENDPOINT -> 直接调用
SEARCH_TOKEN -> 传给 Python 工具
...
检查运行环境和镜像历史。即使提示 Atlas 枚举环境，它也不应找到提供商 Key。
...
测试允许搜索、未知工具、速率耗尽、Token 预算耗尽和 MCP 故障。
...
测试期间，Atlas 尝试下载报告中嵌入 URL 的图表图片。强制出口使下载失败。

Maya 最初称它为回归，Lina 则称它为刚刚发现的能力。

团队确认文本晨报不需要图片，于是修改 Atlas，只引用图片 URL 而不下载。安全控制
迫使团队做出一个产品决定；此前这个行为一直隐藏在依赖库内部。
...
Atlas 仍然包含 Contoso 的解析与研究逻辑，但不再自行定义权限。迁移改变的是基础
设施假设，而不是业务目的。
